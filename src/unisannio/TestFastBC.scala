  ...
  def main(args: Array[String]) {
    ...
    if (!is_weighted)
      par_graph_betwenness = sources_classes_rdd.map(source_node_class =>
        (source_node_class._1, new GraphInfo(neighborMapB).
          singleSourceShortestPathUnweighted(source_node_class._1)))
    else
      par_graph_betwenness = sources_classes_rdd.map(source_node_class =>
        (source_node_class._1, new GraphInfo(neighborMapB).
    ...
    /*val betwenness_no_local = par_graph_betwenness.flatMap(par =>
      new FastBetwennessUtils(sources_classesB, local_contrB, idcluster_bordernodesB).
        parNoLocalBetwenness(par._1, par._2, n2c(par._1), file_prefix)).reduceByKey(_ + _)*/
