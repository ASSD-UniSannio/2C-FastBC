...
object FastBC {
  ...
  val logger = Logger.getLogger("FastBC")
  ...
  def main(args: Array[String]) {
    if (args.length < 7) {
      logger.error("Usage: FastBC <master> <graph_filename> <edge_delimiter> <is_directed> <is_weighted> <n_mappers> <epsilon precision> <output_prefix>")
      System.exit(1)
    }
    ...
    file_prefix = args(7) + "/" + VERSION
    logger.info("*** Executing FastBC with mappers: " + mapper + ", louvain_espilon: " + louvain_epsilon
      + ", log: " + logger.getLevel)
    ...
    val (class_node, local_betweenness_rdd, local_stats) = localClusterStatsEvaluation(sc, n2c,
      clusters_mapped_b, idcluster_bordernodesB, file_prefix, is_weighted)
    ...
    val local_contr = local_stats.map(ls => (ls._1, ls._2.filter(x => x._2 > 0))).
      filter(ls => sources_classes.contains(ls._1)).collectAsMap
    ...
    var pivotsRDD = sc.parallelize(sources_classes.keySet.toSeq)
    if (!is_weighted)
    ...
    val betweenness_no_local = par_graph_betweenness.flatMap(par =>
      new FastBetwennessUtils(sources_classesB, local_contrB, idcluster_bordernodesB).
  ...
  def localClusterStatsEvaluation(
    ...
    file_prefix:           String, is_weighted: Boolean) = {
    ...
    if (!is_weighted) {
      local_stats = sc.parallelize(n2c.toSeq).map(node_comm =>
        new GraphInfo(clusters_mapped_b, idcluster_bordernodes).modifiedSingleSourceShortestPathUnweighted(
          node_comm._1, node_comm._2, file_prefix))
    } else {
      local_stats = sc.parallelize(n2c.toSeq).map(node_comm =>
        new GraphInfo(clusters_mapped_b, idcluster_bordernodes).modifiedSingleSourceShortestPathWeighted(
          node_comm._1, node_comm._2, file_prefix))
    }
    val local_sigma_and_distance = local_stats.map(ls => (ls._1, ls._3.toArray.deep.hashCode))
    //find class of nodes with same specification
