...
object FastBC_2C {
  ...
  def main(args: Array[String]) {
    if (args.length < 10) {
      logger.error("Usage: FastBC <master> <graph_filename> <edge_delimiter> <is_directed> <is_weighted> [<is_weight_distance>]" +
        " <n_mappers> <epsilon precision> <output_prefix> <class_fraction> <num_iterations_kmeans>")
      System.exit(1)
    }
    var is_weight_distance = true //Optional parameter: possible values should be sim and dist
    if(args.length == 11){
       is_weight_distance = args(5).equals("dist") //other value should be sim
       logger.info("There are 11 args. is_weight_distance has value " + is_weight_distance)
    }
    val mapper = args(6).toInt
    val louvain_epsilon = args(7).toDouble
    var file_prefix = ""
    val classFraction = args(9).toDouble
    val num_iterations_kmeans = args(10).toInt

    file_prefix = args(8) + "/" + VERSION + "_" + classFraction + "_class_fraction/"
    logger.info("*** Executing FastBC_2C with mappers: " + mapper + ", louvain_espilon: " + louvain_epsilon + ", weights are distance: " + is_weight_distance
    ...
    /*
     * Metodi per l'inversione del grafo
     */
//    var max_weight2=1    
//    max_weight2 = graph.edges.map(_.attr).max
//    louvainGraph = Graph(graph.vertices, graph.edges.map(e => Edge(e.srcId, e.dstId, -e.attr + 1 + max_weight2)))
   
    
    val runner = new LouvainMethod()
    //weights in Louvain has always to be inverted... (in the new configuration of the command line arguments)
    val (n2c, maximumWeight) = runner.run(louvainGraph, sc, louvain_epsilon, true, new LouvainConfig(file_prefix))
    ...
    val sources_classes = class_node.map(cn => new FastBetwennessUtils().selectSource(
      cn.toArray, local_betweenness, idcluster_bordernodes(n2c(cn.toArray.apply(0))))).collectAsMap
    ...
    var par_graph_betweenness: RDD[(Long, scala.collection.mutable.Map[Long, Double])] = null
    var pivotsRDD = sc.parallelize(sources_classes.keys.toSeq)

    if (!is_weighted)
      par_graph_betweenness = pivotsRDD.map(pivot =>
        (pivot, new GraphInfo(neighborMapB).singleSourceShortestPathUnweighted(pivot)))
    else
      par_graph_betweenness = pivotsRDD.map(pivot =>
        (pivot, new GraphInfo(neighborMapB).singleSourceShortestPathWeighted(pivot)))

    val brandes_2 = System.currentTimeMillis
    logger.info("*** SSSPs performed over " + sources_classes.size + " pivot nodes (using "
      + pivotsRDD.partitions.size + " RDD partitions) took " + (brandes_2 - brandes_1) / 1000.0 + " s")
    ...
    val no_local_bc_1 = System.currentTimeMillis

    val betweenness_no_local = par_graph_betweenness.flatMap(par => new FastBetwennessUtils(sources_classesB, local_contrB, idcluster_bordernodesB).
      parNoLocalBetwenness(par._1, par._2, n2c(par._1))).reduceByKey(_ + _)
    val no_local_bc_2 = System.currentTimeMillis

    logger.info("*** No Local BC inside fix2-fix1 took " + (no_local_bc_2 - no_local_bc_1) / 1000.0 + " s")
    
    val fix_2 = System.currentTimeMillis
    logger.info("*** Fixing partial BC values from pivot with cardinality and local BC took "
      + (fix_2 - fix_1) / 1000.0 + " s")

    val bc_reduce_1 = System.currentTimeMillis
    val final_betweenness = (betweenness_no_local ++ local_betweenness_rdd).reduceByKey(_ + _).sortBy(-_._2).collect
    ...
    if (!is_directed) //divide by two the BC values if undirected
      for (i <- 0 until final_betweenness.length)
        final_betweenness(i) = (final_betweenness(i)._1, final_betweenness(i)._2 / 2.0)
    val betweennesstime = System.currentTimeMillis - startTime
    writeBetweennessCentralityToFile(file_prefix, "result.csv", final_betweenness)
