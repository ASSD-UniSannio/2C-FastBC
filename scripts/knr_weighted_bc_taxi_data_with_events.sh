...
HOME='/home/furno/proj3_fast_bc_weighted/'
...
OUTPUT_FOLDER=$HOME'results/knr_interpolated_rhone_alpes/results_with_events_with_non_imposed_partitions'
...
# -------------
MASTER_URI='spark://TICIL.ifsttar.fr:7077'
...
declare -a NUM_CORES_PER_EXECUTOR_LIST=(28 24 20 16 8 4 2 1)

for NUM_CORES_PER_EXECUTOR in ${NUM_CORES_PER_EXECUTOR_LIST[@]}; do
	TOTAL_NUM_CORES=$(expr $NUM_CORES_PER_EXECUTOR \* $NUM_EXECUTORS)
	echo "### ANALYZING INPUT: "$INPUT_FILEPATH
	echo "### WRITING OUTPUTS TO: "$OUTPUT_FOLDER
	...
	do
		...
		spark-submit \
			...
			--conf spark.eventLog.dir='/home/furno/spark-2.2.0-bin-hadoop2.7/event_log' \
			$JAR_FILE \
			$MASTER_URI \
